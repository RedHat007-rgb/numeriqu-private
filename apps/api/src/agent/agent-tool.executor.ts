...
export class AgentToolExecutor {
   ...
   */
  async generateDashboard(tenantId: string, userId: string, params: {
    title: string;
    description?: string;
    charts: any[];
  }) {
    this.logger.log(`[Agent:SaveDashboard] Creating "${params.title}" for tenant=${tenantId}`);
    ...
    try {
      ...
      return {
        ...
        message: `Pinned "${params.title}" to your dashboard.`,
      };
    } catch (error: any) {
      this.logger.error(`[Agent:SaveDashboard] Failed: ${error.message}`);
      throw new Error(`DATABASE_ERROR: Could not save dashboard entity.`);
    }
  }
  ...
  async fetchSpecificMetrics(tenantId: string, params: {
    ...
    this.logger.debug(`[Agent:FetchMetrics] ${params.metric} for tenant=${tenantId}`);
    ...
    try {
      ...
      throw new Error(`DATA_FETCH_ERROR: Could not retrieve ${params.metric} data.`);
    }
  }
}
