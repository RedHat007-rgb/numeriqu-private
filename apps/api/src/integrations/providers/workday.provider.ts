...
export class WorkdayProvider implements IntegrationProvider {
  ...
  async triggerSync(jobDetails: SyncJobConfig, connectionId: string): Promise<number | void> {
    this.logger.log(`[Workday] Starting WQL extraction job: ${jobDetails.syncJobId}`);
    ...
    if (!connection || !connection.refreshToken) {
      throw new Error(`Workday connection ${connectionId} is missing valid credentials.`);
    }
    ...
    try {
      ...
      if (connection.metadata && typeof connection.metadata === 'string') {
          ...
          const meta = this.crypto.decryptJson(connection.metadata as string);
          host = meta.host;
      ...
      const queries = {
        workers: 'SELECT workerID, legalName, workEmail, positionTitle FROM workers',
        journals: 'SELECT journalEntryID, company, ledger, totalAmount FROM journalEntries',
        purchaseOrders: 'SELECT purchaseOrderID, documentDate, totalAmount FROM purchaseOrders'
      };
      ...
      for (const [entity, query] of Object.entries(queries)) {
        ...
        const data = await this.executeWQL(host!, workdayTenantId!, accessToken, query);
      ...
      this.logger.log(`[Workday] Successfully ingested ${totalRecords} records.`);
      return totalRecords;
    } catch (e: any) {
  ...
  private async executeWQL(host: string, tenantId: string, token: string, query: string): Promise<any[]> {
  ...
  private async streamToClickHouse(internalTenantId: string, entity: string, records: any[]) {
