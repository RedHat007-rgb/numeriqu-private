...
export class AgentToolsService {
   ...
   */
  async saveInsightToDashboard(tenantId: string, params: {
    title: string;
    description?: string;
    ...
    this.logger.log(`[Tool:SaveInsight] Creating "${params.title}" for tenant=${tenantId}`);
    ...
    try {
      ...
      throw new Error(`DATABASE_ERROR: I couldn't save this chart to your dashboard. Please try again or contact support if this persists.`);
    }
  }
   ...
   */
  async queryFinancialDatabase(tenantId: string, params: { sql: string; reason: string }) {
    this.logger.log(`[Tool:SQL] Executing analysis for: "${params.reason}"`);
    
    try {
      const rows = await this.financialData.executeScopedQuery(tenantId, params.sql);
      return {
        success: true,
        data: rows,
        count: rows.length,
      ...
      throw new Error(`ANALYSIS_FAULT: My investigation encounterd a technical hurdle while querying the live ledger.`);
    }
  }
   ...
   */
  async fetchSpecificMetrics(tenantId: string, params: {
    metric: 'revenue' | 'expenses' | 'profit' | 'invoices' | 'cashflow';
    timeRange?: string;
    orgId?: string;
  }) {
    this.logger.debug(`[Tool:FetchMetrics] ${params.metric} for tenant=${tenantId}`);
    ...
    try {
      ...
      switch (params.metric) {
        case 'revenue':
          return { data: profile.revenue, trend: await this.financialData.getMonthlyRevenueTrend(tenantId) };
        case 'expenses':
      ...
      throw new Error(`DATA_FETCH_ERROR: I encountered an issue retrieving your ${params.metric} data from the analytics engine.`);
    }
  }
}
