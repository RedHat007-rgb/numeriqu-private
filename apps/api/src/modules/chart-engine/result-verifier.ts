 ...
 */
export function reconcileAdditive(parts: number[], charted: number, tolerance = 0.01): Reconciliation {
  const recomputed = parts.reduce((a, b) => a + (Number.isFinite(b) ? b : 0), 0);
  const denom = Math.max(Math.abs(charted), 1e-9);
  const relDelta = Math.abs(recomputed - charted) / denom;
  return { ok: relDelta <= tolerance, recomputed, charted, relDelta, tolerance };
}
...
export function verifyScoped(sql: string): { ok: boolean; reason?: string } {
  ...
  const hasOrg = /org_id\s+IN\s+\(\{externalOrgIds:Array\(String\)\}\)/i.test(sql);
  if (hasTenant && hasOrg) return { ok: true };
  return { ok: false, reason: `query is not tenant-scoped (expected: ${SCOPE_WHERE})` };
}

/** Pick the reconciliation appropriate to a measure's expression kind. */
export function reconcileForExpr(
  expr: MeasureExpr,
  data: { parts?: number[]; sumNumerator?: number; sumDenominator?: number; charted: number },
  ...
  switch (expr.kind) {
    ...
    case 'mean':
      ...
      // a stock is a level, a distinct-count doesn't add); scope check still applies.
