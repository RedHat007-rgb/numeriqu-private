import { compileSpec, compileNameValueSql, compileSeriesSql, buildEngineDisplay, SCOPE_WHERE } from './spec-compiler';
...
const model: SemanticModel = {
  ...
  dimensions: [
    ...
    { key: 'business_unit', label: 'Business Unit', table: 'v_fact', column: 'business_unit' },
  ],
  ...
  measures: [
    ...
    { key: 'gross_margin_pct', label: 'Gross Margin %', unit: '%', sourceTable: 'v_fact', expr: { kind: 'ratio_of_sums', numerator: 'gross_profit_usd', denominator: 'revenue_usd' } },
    { key: 'cash_balance', label: 'Cash Balance', unit: 'USD', sourceTable: 'v_fact', expr: { kind: 'last_value', column: 'cash_usd', orderBy: 'period_date' } },
    { key: 'clients', label: 'Clients', unit: 'count', sourceTable: 'v_fact', expr: { kind: 'count_distinct', column: 'client_id' } },
    { key: 'dso_days', label: 'Dso Days', unit: 'days', sourceTable: 'v_fact', expr: { kind: 'mean', column: 'dso_days' } },
    { key: 'sla_pct', label: 'Sla %', unit: '%', sourceTable: 'v_fact', expr: { kind: 'mean', column: 'sla_pct_sum', weight: 'sla_pct_wt' } },
  ],
};

const ctx = { analyticsDb: 'analytics', tenantId: 'org-uuid', externalOrgIds: ['ext-1', 'ext-2'] };

describe('SpecCompiler tenant scoping', () => {
  it('always binds tenant_id + org_id as parameters, never interpolated', () => {
    const spec: EngineChartSpec = { chartType: 'bar', measureKeys: ['revenue'], dimensionKey: 'business_unit', title: 'Revenue by BU' };
    const r = compileSpec(spec, model, ctx);
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    expect(r.sql).toContain(SCOPE_WHERE);
    expect(r.params).toEqual({ tenantId: 'org-uuid', externalOrgIds: ['ext-1', 'ext-2'] });
...
describe('SpecCompiler correctness contract', () => {
  it('compiles a ratio as SUM/SUM — never an average', () => {
    const spec: EngineChartSpec = { chartType: 'bar', measureKeys: ['gross_margin_pct'], dimensionKey: 'business_unit', title: 'Margin by BU' };
    const r = compileSpec(spec, model, ctx);
    expect(r.ok).toBe(true);
    if (!r.ok) return;
  ...
  it('compiles a stock (cash balance) as argMax over time — never summed', () => {
    const spec: EngineChartSpec = { chartType: 'kpi', measureKeys: ['cash_balance'], title: 'Cash' };
    const r = compileSpec(spec, model, ctx);
    expect(r.ok).toBe(true);
    if (!r.ok) return;
  ...
  it('compiles count_distinct as uniqExact', () => {
    const spec: EngineChartSpec = { chartType: 'kpi', measureKeys: ['clients'], title: 'Clients' };
    const r = compileSpec(spec, model, ctx);
  ...
  it('compiles an unweighted mean (duration) as avg — never summed, never ×100 scaled', () => {
    const spec: EngineChartSpec = { chartType: 'bar', measureKeys: ['dso_days'], dimensionKey: 'business_unit', title: 'DSO by BU' };
  ...
  it('compiles a WEIGHTED mean (pre-summed % + weight) as sum/sum, not avg-of-averages, not ×100', () => {
    const spec: EngineChartSpec = { chartType: 'bar', measureKeys: ['sla_pct'], dimensionKey: 'business_unit', title: 'SLA by BU' };
    const r = compileNameValueSql(spec, model, ctx);
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    // Already in percent-points, so SUM(value)/SUM(weight) with NO ×100 scaling.
    expect(r.sql).toContain('sum(sla_pct_sum) / nullIf(sum(sla_pct_wt), 0) AS value');
...
describe('SpecCompiler grouping, time, top-N', () => {
  it('emits a monthly time grain with ORDER BY period', () => {
    const spec: EngineChartSpec = { chartType: 'line', measureKeys: ['revenue'], timeGrain: 'month', title: 'Revenue trend' };
    const r = compileSpec(spec, model, ctx);
    expect(r.ok).toBe(true);
    if (!r.ok) return;
  ...
  it('emits ORDER BY measure DESC + LIMIT for top-N', () => {
    const spec: EngineChartSpec = { chartType: 'bar', measureKeys: ['revenue'], dimensionKey: 'client', topN: 5, title: 'Top 5 clients' };
    const r = compileSpec(spec, model, ctx);
    expect(r.ok).toBe(true);
    if (!r.ok) return;
  ...
  it('never aliases an aggregate to its source column name', () => {
    ...
    const r = compileSpec(
      { chartType: 'bar', measureKeys: ['revenue_usd'], dimensionKey: 'business_unit', topN: 5, title: 'Revenue' },
      collisionModel,
      ctx,
    );
    expect(r.ok).toBe(true);
...
describe('compileNameValueSql axis labels', () => {
  it('emits a HUMAN month label and sorts chronologically by the date bucket', () => {
    ...
    expect(r.sql).toContain("formatDateTime(toStartOfMonth(period_date), '%b %Y') AS name");
  ...
  it('emits Q1 2025-style quarter labels', () => {
    const spec: EngineChartSpec = { chartType: 'bar', measureKeys: ['revenue'], timeGrain: 'quarter', title: 'Quarterly revenue' };
    const r = compileNameValueSql(spec, model, ctx);
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    expect(r.sql).toContain("concat('Q', toString(toQuarter(toStartOfQuarter(period_date)))");
  });
...
describe('compileSeriesSql multi-series output', () => {
  ...
  it('emits the explicit x/y/z transport contract for point charts', () => {
    ...
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    expect(r.sql).toContain("concat(toString(client_name), ' — ', toString(business_unit)) AS name");
  ...
  it('emits name + one aliased column PER measure and no `value` column', () => {
    const spec: EngineChartSpec = { chartType: 'bar', measureKeys: ['revenue', 'gross_margin_pct'], timeGrain: 'month', title: 'Revenue & Margin' };
    const r = compileSeriesSql(spec, model, ctx);
    expect(r.ok).toBe(true);
    if (!r.ok) return;
  ...
  it('orders a multi-measure dimension breakdown by the FIRST measure DESC', () => {
    const spec: EngineChartSpec = { chartType: 'bar', measureKeys: ['revenue', 'gross_margin_pct'], dimensionKey: 'business_unit', title: 'x' };
    const r = compileSeriesSql(spec, model, ctx);
  ...
  it('emits generic long-form rows for a monthly series breakdown', () => {
    ...
    expect(r.sql).toContain('GROUP BY toStartOfMonth(period_date), business_unit');
...
describe('buildEngineDisplay axis assignment', () => {
  it('provides two named series for a previous-year comparison', () => {
    ...
    expect(d.series?.map((series) => series.key)).toEqual(['Current Year', 'Previous Year']);
  ...
  it('single measure → just a value format, no series', () => {
    const spec: EngineChartSpec = { chartType: 'bar', measureKeys: ['revenue'], dimensionKey: 'business_unit', title: 'x' };
    const d = buildEngineDisplay(spec, model);
  ...
  it('a $ measure + a % measure → dual-axis COMBO (%, line, right)', () => {
    const spec: EngineChartSpec = { chartType: 'bar', measureKeys: ['revenue', 'gross_margin_pct'], timeGrain: 'month', title: 'x' };
    const d = buildEngineDisplay(spec, model);
  ...
  it('two same-unit ($) measures → both left-axis bars, stays a clustered bar (not combo)', () => {
    const twoMoney = { ...model, measures: [...model.measures, { key: 'cost', label: 'Cost', unit: 'USD', sourceTable: 'v_fact', expr: { kind: 'sum', column: 'cost_usd' } as const }] };
    const spec: EngineChartSpec = { chartType: 'bar', measureKeys: ['revenue', 'cost'], dimensionKey: 'business_unit', title: 'x' };
    const d = buildEngineDisplay(spec, twoMoney);
    expect(d.chartType).toBe('bar');
    expect(d.series?.every((s) => s.axis === 'left' && s.role === 'bar')).toBe(true);
  });
});

describe('SpecCompiler honest refusals', () => {
  it('refuses an unknown measure instead of guessing', () => {
    const spec: EngineChartSpec = { chartType: 'bar', measureKeys: ['ebitda'], title: 'x' };
    const r = compileSpec(spec, model, ctx);
  ...
  it('refuses a cross-table query in v1 rather than joining blindly', () => {
    const twoTable: SemanticModel = {
      ...
      measures: [
        ...model.measures,
        { key: 'other', label: 'Other', unit: 'USD', sourceTable: 'v_other', expr: { kind: 'sum', column: 'amount' } },
      ],
    };
    const spec: EngineChartSpec = { chartType: 'bar', measureKeys: ['revenue', 'other'], title: 'x' };
